...
function uni_setting($uniacid = 0, $fields = '*') {
	...
	$serialize = array('site_info', 'menuset', 'sync', 'stat', 'oauth', 'passport', 'uc', 'notify', 'creditnames', 'default_message', 'creditbehaviors', 'shortcuts', 'quickmenu', 'payment', 'groupdata');
	if($fields == '*') {
	} else {
		if(!is_array($fields)) {
			$fields = array($fields);

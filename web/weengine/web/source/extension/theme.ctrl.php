...
$dos = array('installed', 'prepared', 'install', 'refresh', 'uninstall', 'web', 'batch-install', 'designer', 'check');
	...
	if($_W['ispost']) {
		...
		if (pdo_insert('site_templates', $m)) {
			...
			$id = pdo_insertid();
						...
						if(!empty($m['settings'])) {
				foreach($m['settings'] as $variable => $value) {
...
if($do == 'install') {
	...
	if (pdo_fetchcolumn("SELECT id FROM ".tablename('site_templates')." WHERE name = '{$manifest['name']}'")) {
		...
		$newstyle = array(
			'uniacid' => $_W['uniacid'],
			'name' => $manifest['title'] . '_' . random(4),
			'templateid' => $templateid
		);
		pdo_insert('site_styles', $newstyle);
		$id = pdo_insertid();
			...
			foreach($manifest['settings'] as $variable => $value) {
				if(!empty($value) && empty($styles[$variable])) {
					pdo_insert('site_styles_vars', array(
						'content' => $value,
						'templateid' => $templateid,
...
if ($do == 'designer') {
	...
	if (checksubmit('submit') && $available[$_GPC['method']]) {
		...
		$t['template']['version'] = trim($_GPC['template']['version']);
		if(empty($t['template']['version']) || !preg_match('/^[\d\.]+$/i', $t['template']['version'])) {
			message('必须输入模板版本号(仅支持数字和句点). ');
		}
		...
		foreach($_GPC['settings']['variables'] as $key => $value) {
			...
			if(!empty($_GPC['settings']['variables'][$key]) && preg_match('/^[a-z\d]+$/i', $_GPC['settings']['variables'][$key])) {
				if (!empty($_GPC['settings']['values'][$key]) && preg_match('/^[a-z\d]+$/i', $_GPC['settings']['values'][$key])) {
...
function manifest($t) {
	...
	foreach($t['settings'] as $key => $value) {
		$item .= "\r\n\t\t<item variable=\"{$value['variable']}\" content=\"{$value['value']}\" />";
	}
	$tpl = <<<TPL
<?xml version="1.0" encoding="utf-8"?>
<manifest versionCode="{$versions}">
	<identifie><![CDATA[{$t['template']['name']}]]></identifie>
	<title><![CDATA[{$t['template']['identifie']}]]></title>

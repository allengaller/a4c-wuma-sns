...
if($do == 'post') {
	if ($_W['isajax'] && $_W['ispost']) {
		...
		$sql = 'SELECT `rid` FROM ' . tablename('rule_keyword') . " WHERE `uniacid` = :uniacid AND `module` = :module AND `content` = :content";
		$result = pdo_fetch($sql, array(':uniacid' => $_W['uniacid'], ':module' => $m, ':content' => $_GPC['keyword']));
		if (!empty($result)) {
			$sql = 'SELECT `id`, `name` FROM ' . tablename('rule') . " WHERE `id` = :id";
			$rule = pdo_fetch($sql, array(':id' => $result['rid']));
			exit(@json_encode($rule));
		}
		exit('success');
	}
	$rid = intval($_GPC['rid']);

...
<script type="text/javascript">
	...
	function submitform() {
		...
		var id = $('#account').val();
		$.ajax('/api.php?flag=1&id=' + id, {
			type : 'POST',
			headers : {"Content-type" : "text/xml"},
			data : $('#sendxml').val().replace(/[\r\n]/g,""),
			success : function(data) {
				var data = $.parseJSON(data);
				var url = '/api.php?encrypt_type=' + data.encrypt_type + '&id=' + id + '&timestamp=' + data.timestamp + '&nonce=' + data.nonce + '&signature=' + data.signature + '&msg_signature=' + data.msg_signature;

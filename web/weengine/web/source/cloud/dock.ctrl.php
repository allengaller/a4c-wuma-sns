 ...
 */
load()->model('cloud');
$dos = array('auth', 'build', 'schema', 'download', 'module.query', 'module.info', 'module.build', 'theme.query', 'theme.info', 'theme.build');
...
if(in_array($do, array('module.query', 'module.info', 'module.build', 'theme.query', 'theme.info', 'theme.build'))) {
	$dat = __secure_decode($post);
	if(!empty($dat)) {
		$secret = random(32);
		$ret = array();

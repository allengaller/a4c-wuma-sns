...
function site_navs($params = array()) {
	...
	$navs = pdo_fetchall("SELECT id, name, description, url, icon, css, position, module FROM ".tablename('site_nav')." WHERE position = '1' AND status = 1 AND uniacid = '{$_W['uniacid']}' AND multiid = '{$multi['id']}' $condition ORDER BY displayorder DESC");
	if (!empty($navs)) {
		foreach ($navs as &$row) {
